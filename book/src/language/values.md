...
*non-cons* value as a one-element sequence containing itself — useful, and
occasionally surprising.

## Truthiness

rizz has **no boolean type**. Conditionals (`if`, `not`, `and`, `or`, `cond`,
…) ask whether a value is *truthy*. The following values are **false**:
...
A [`ref`](refs.md) is truthy exactly when its *contents* are truthy — this is
...
NaN propagates silently through float arithmetic, but it is *rejected* wherever
...
This, truthiness, and a handful of other contexts are the *only* places a ref is
auto-peeled. The full list is in [Refs and Mutability](refs.md).

---

*See also:* [Refs and Mutability](refs.md) · [Collections](collections.md) ·
[The Standard Library](stdlib.md) · *SPEC.md* §3
