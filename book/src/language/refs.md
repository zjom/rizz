...
_only_ path to mutation; everything else stays immutable.
...
These commit a result back into a cell. For _non-mutating_ updates, use the
...
fixed set of contexts look _through_ a ref automatically. Know this list,
because everything _not_ on it does not peel:
...
| `empty-of`                | Returns the zero of the _contents_, not a fresh ref.             |
...
2. **No auto-collapse on construction.** `ref`, `set!`, `push!`, `put!`, and the
   rest store _exactly_ what you hand them. Wrapping a ref in a ref gives you two
...
threading refs through deep call chains. [Patterns and Anti-patterns](../idioms/patterns.md) has more on when a ref earns its keep.
